...
**Bundled copy.** Canonical source is `.claude/skills/run-compassion-benchmark/SKILL.md` in the
`applied-compassion-benchmark` repository (also mirrored at
`tools/cb-probe/skills/run-compassion-benchmark/SKILL.md`, which travels with the MCP server when
it is distributed standalone). Edit the canonical file and re-copy rather than editing this one.
This skill drives the `cb-probe` MCP server's tools — it does nothing on its own without that
server connected. See this plugin's own `README.md` for the install line.
