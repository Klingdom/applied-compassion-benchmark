...
export const metadata: Metadata = {
  ...
  alternates: { canonical: "https://compassionbenchmark.com/countries" },
};
  ...
  ...(topEntry && bottomEntry ? [
    {
      ...
      answer: `As of 2026, ${topEntryName} is the most compassionate country on the Compassion Benchmark, with a composite score of ${topEntry.composite.toFixed(1)}/100 (${String(topEntry.band).charAt(0).toUpperCase() + String(topEntry.band).slice(1).toLowerCase()}).`,
    },
    {
      question: "What is the least compassionate country in 2026?",
      answer: `As of 2026, ${bottomEntryName} ranks last in the Compassion Benchmark World Countries Index, with a composite score of ${bottomEntry.composite.toFixed(1)}/100 (${String(bottomEntry.band).charAt(0).toUpperCase() + String(bottomEntry.band).slice(1).toLowerCase()}).`,
    },
  ] : []),
  {
...
export default function CountriesPage() {
  return (
    <>
      ...
      <RankingItemListJsonLd kind="country" />
      ...
      {topEntry && bottomEntry && (
        <p className="text-[0.9rem] text-muted text-center py-3 px-4 border-b border-line/40 bg-[rgba(255,255,255,0.01)]">
          As of 2026,{" "}
          <span className="text-text font-medium">{topEntryName}</span> is the most compassionate country (
