...
**Last updated:** 2026-09-07. WQ-P1-07 (the product-separation guard) is now **DONE** — see its row
below. Everything else in this queue is still not started.
...
| WQ-P1-07 | **Build the separation guard.** A validator asserting no model score and no lab score share an aggregate, and no entity holds two published composites anywhere. | backend-engineer | none — **buildable now** | `01-REPOSITORY-AND-PLATFORM-BUILD` required acceptance test; §2.1 shows three live double-publications | The guard runs inside the existing `validate-indexes.mjs` gate and **currently fails** on Microsoft/Amazon/Meta — which is the correct first result | **DONE — 2026-09-07.** `site/scripts/validate-product-separation.mjs` + `lib/product-separation.mjs` + `lib/deployed-ai-audit-subjects.mjs` + `test-product-separation.mjs` (16/16 fixture tests pass), wired into `npm test`. **Not** wired into `validate-indexes.mjs`/`npm run build` — see `DECISIONS.md` D-23 consequence note (would block every build on pre-existing, un-triaged defects). Runs standalone via `npm run validate:product-separation`. First-run result against live data: **FAIL**, 8 blocking findings (2 name fusions, 6 duplicate-composite groups), 10 warnings (deployed products inside `ai-labs.json`), check 4 (model discriminator) correctly vacuous — no model index exists yet. Full finding list: `DECISIONS.md` D-23. |
