...
### 2.7A Addendum — L1 can now read what it fetches (implemented 2026-09-18, backend-engineer)

Iteration 17 shipped the L1 fetch loop honestly incomplete: `release-watch-l1.mjs`'s own header said, in plain words, "does not parse retrieved bytes." It could enumerate `release-sources-v1.json`, GET each URL, and hash the response — but a reached source could never become a candidate, because nothing turned bytes into an item. This addendum documents what closes that gap. It changes no schema field that existed before, adds two new pure modules, adds two new scan-record fields, and does not touch `release-sources-v1.json` (still 0 entries; R6/R11 remain a human's job) or `releases-v1.json` (still empty, still `scanState: "never-scanned"`, still `coverageClaim: "none"` — nothing in this addendum runs a real scan yet, because there is still nothing to fetch).

**New modules, both pure (no I/O, no network, fixture-tested offline):**

| Module | Job |
|---|---|
| `research/scripts/lib/release-watch-parse.mjs` | Turns fetched bytes (a string) into raw dated items (RSS 2.0, Atom, JSON Feed, or a conservative HTML-listing fallback), applies the fail-closed candidate bar, and reports the release-recognition rule's verdict per candidate. |
| `research/scripts/lib/release-watch-state.mjs` | Per-source dedupe against previously-seen items, and filtering out candidates already promoted into a confirmed release. |

**Formats parsed, and each one's fail-closed behaviour:**

- **RSS 2.0** — a small regex-based `<item>` extractor (NOT a conformant XML parser; the module's header states its limits: no nested `<item>`, no XML-namespace-prefixed item tags, first-match-only per field). `pubDate` (or `dc:date`) is required; absent ⇒ dropped, reason `no-date`.
- **Atom** — same class of tolerant extractor over `<entry>`. Prefers `<published>`, falls back to `<updated>` (legal Atom). Absent ⇒ dropped, reason `no-date`. The first `<link href="…">` found is used regardless of `rel` — a documented limitation, not a silent one.
- **JSON Feed 1.1** — real `JSON.parse` (no hand-rolled tolerance needed for JSON). `date_published` required; absent ⇒ dropped, reason `no-date`. `url` preferred as the link; `id` used only as a fallback and only when it is itself an `http(s)://` URL.
- **HTML-listing fallback** — the conservative case the task named explicitly. Pairs a link with a date found on the **same line of raw text only**; two date shapes recognised (`YYYY-MM-DD`, `Month D, YYYY`). A link with no date on its own line produces **nothing** — no DOM tree, no proximity search across lines, no guess. This is the literal fail-closed requirement: "if it cannot find a date it yields no candidate rather than guessing."
- Every format, plus a defensive top-level `parseSourceBytes()` catch, degrades to **zero items** on malformed input rather than throwing — proven against a deliberately truncated RSS fixture and a deliberately malformed JSON Feed body.

**The fail-closed candidate bar (extraction), stricter than the two conditions named in the task:** a raw item becomes a `candidates[]` entry only if it has ALL THREE of a parseable ISO date, a non-empty title, **and** a non-empty link (link was added beyond the task's two named conditions, because a candidate's link is what becomes its evidence `source_url` if ever promoted — an unattributable item is not evidence). Missing any one produces a `dropped_candidates[]` entry (mirroring the nightly entity scanner's `research/scans/*.json` → `dropped_candidates: [{ …, reason }]` convention) carrying `source_id`, `reason`, and whatever of `title`/`link`/`snippet` is known. Each candidate carries `source_id`, `title`, `link`, `published_at` (ISO), and the verbatim `snippet` it came from, per the task's contract.

**The recognition rule, in one paragraph, with its failure modes:** `recognizeRelease()` marks `is_release_candidate: true` only when a candidate's title+snippet matches BOTH at least one entry from a documented nine-phrase keyword list (`announcing`, `introducing`, `release`, `launch`, `now available`, `general availability`, `unveil`, `debut`, `ships`) AND a required version-or-name token regex (`VERSION_OR_NAME_TOKEN_RE` — catches shapes like `4.7`, `v2.1`, `GPT-5`); it reports a `confidence` in `(0, 0.9]` scaling with the number of matched keywords, and it **never drops a candidate and never promotes one** — every extracted item reaches `candidates[]` regardless of the verdict, exactly as the task requires ("route everything to candidates[] for human promotion — never auto-promote"). Known false positives: a keyword next to an unrelated numeric token ("launching our Q3 2026 roadmap"), or a retrospective post reusing release vocabulary ("looking back at how GPT-4 launched" — pinned by a test). Known false negatives: an announcement verb outside the nine-phrase list ("is live today", "drops today"), or a name with no digits at all ("Codename Meridian") — both still reach `candidates[]` with `is_release_candidate: false`, so a human still sees them; the rule only annotates, it filters nothing out of the record.

**Dedupe and "already promoted," and where state lives:** `research/model-index/release-watch/state/source-state.json` — a **subdirectory** of the scan-record tree, deliberately: `validate-model-releases.mjs` treats every top-level `.json` file directly in `research/model-index/release-watch/` as a scan record (`readdirSync(...).filter(f => f.endsWith(".json"))`); nesting the state file one level down keeps it invisible to that sweep, which is a correctness requirement, not a tidiness choice. Per source, the state records `seenKeys` (dedupe key: the item's link, or a `source+title+date` composite when no link exists), `lastCheckedAt`, `etag`/`lastModified` (reserved fields for a future conditional-GET optimisation; unused by this revision, which always does a full GET), and `lastItemPublishedAt`. A candidate whose key is already in `seenKeys` is dropped with reason `already-seen` — not re-reported. Separately, `filterAlreadyPromoted()` reads (read-only) `releases-v1.json`'s evidence `source_url`s and drops any candidate whose link already appears there, reason `already-promoted` — so a candidate that has already gone through the human T3/T4 gate is never re-offered. Neither mechanism touches `releases-v1.json`; both are pure functions over data the CLI reads and writes.

**New scan-record fields (additive; every field §2.6 already specified is unchanged):** `dropped_candidates[]` (the itemised-drop list above) and `fixture_run: boolean`. `candidates[]` entries now carry, beyond the §2.3-adjacent evidence-shaped fields, `format`, `is_release_candidate`, `confidence`, `matched_keywords`, `matched_version_token`, and `recognition_reasons` — all advisory, all for the human reviewing the record, none of it ever copied into `releases-v1.json` automatically.

**`--fixture-run` — proof without live sources.** `release-watch-l1.mjs --fixture-run` performs zero network I/O and runs the identical `runL1Scan()` core a real `--live` run uses, substituted only at the fetch boundary: a local-file-reading shim stands in for `globalThis.fetch`, pointed at a small, clearly-synthetic in-memory source registry (`fixture-rss`, `fixture-atom`, `fixture-jsonfeed`, `fixture-html-dated`, `fixture-html-no-dates`, `fixture-malformed`, `fixture-empty`) whose "URLs" resolve to files under `research/scripts/fixtures/release-watch/`. Because the substitution happens only at the fetch boundary, this is a real exercise of the production parse/extract/recognise/dedupe path, not a parallel mock of it. Its output is written to `research/model-index/release-watch/fixture-runs/` (a second subdirectory invisible to `validate-model-releases.mjs`'s top-level scan-record sweep, for the same reason as the state directory) with its own `fixture-runs/source-state.json`, and every record it writes carries `fixture_run: true`. It never advances, and can never be mistaken for, real coverage: `releases-v1.json`'s `scanState`/`coverageClaim` are computed only from the top-level scan-record tree, which a fixture run never writes to.

### 2.7B Daily integration — the release-watch step (implemented 2026-09-18)

Per Track R's own framing (§8): release-watch has **no dependency on the entity-scoring nightly chain** (`overnight-scanner → overnight-assessor → overnight-digest → validation`). It is documented here as an **independent daily step**, not an insertion into that chain, and it must not become one — release-watch candidates are not entity assessments and must never touch `rotation-state.json`, `research/change-proposals/`, or any index.

| | |
|---|---|
| **Runs** | `node research/scripts/release-watch-l1.mjs --live`, once daily, as part of the same automation window as the nightly entity chain (see `AGENT-ROUTING.md` §1 for the sequencing note) but as a separate invocation with no data dependency in either direction. |
| **Runs today, in practice** | With `release-sources-v1.json` at 0 entries, the zero-sources check fires before the `--live`/dry-run branch even matters, so today's daily `--live` invocation writes an honest `status: "not-run"`, `blocked_by: "no-sources-registered: …"` record — **identical in substance to today's manual runs**. This is deliberate: the daily wiring can go live *today*, at zero risk, and will start producing real fetches automatically the day a human completes R6/R11 — no further wiring change required. |
| **Writes** | One scan record per run to `research/model-index/release-watch/<scan_id>.json` (§2.6, extended per §2.7A); per-source dedupe state to `research/model-index/release-watch/state/source-state.json`. Never writes `releases-v1.json`. Never writes `rotation-state.json`, `research/scans/**`, `research/change-proposals/**`, or any index. |
| **Gate it must pass** | `validate-model-releases.mjs` (already wired into `npm test`/`npm run build`, unchanged by this addendum) — K16 scan-record internal consistency, K12 bidirectional FK once `releases-v1.json` has rows. Promotion of a `candidates[]` entry into a confirmed release remains T3 (human, ≥2 sources or 1 primary-provider source) then T4 (**human merge only**, §2.5) — this addendum adds candidates to review; it adds no new promotion path and no automation touches `releases-v1.json`. `score-updater`-style human gating is unchanged and unextended. |
| **What the digest reports** | `overnight-digest` does not currently read release-watch output at all (not in its §1 "Reads" column in `AGENT-ROUTING.md`) — this addendum specifies, but does not implement, the contract a future digest change must satisfy: **name today's release-watch outcome explicitly, every day, even when it is empty.** While `release-sources-v1.json` has 0 entries the required line is literally *"Release watch: 0 sources registered — detection did not run today."* Silence on this point is not neutral: a digest that says nothing about release-watch reads, to a founder skimming daily output, as "no releases" rather than as "not monitored" — the exact ambiguity C1/§2.4's `scanState`/`coverageClaim` fields exist to prevent inside the data layer. The digest is the human-facing surface where that same honesty must be repeated in prose, sourced from the day's scan record's `status`/`blocked_by` fields (or, once sources exist, from `sources_reached.length`, `candidates.length`, and `dropped_candidates.length`), never authored by hand. Implementing this line in `overnight-digest`'s own spec is out of this task's file-ownership scope (`.claude/agents/overnight-digest.md` is not an owned file here) and is flagged as follow-up work for whoever owns that agent definition. |
| **Who runs it** | Same automation context as the rest of the nightly run (a deterministic script, not an agent exercising judgment) — no agent-level authority is exercised by this step beyond what `release-watch-l1.mjs` already had. Reviewing `candidates[]` for possible promotion is a human/founder task, or a future research-agent role not yet defined; this addendum does not assign it, because doing so is a product/process decision outside a backend task's scope. |
