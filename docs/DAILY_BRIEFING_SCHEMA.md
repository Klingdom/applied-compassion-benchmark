...
**Unapplied-score-movement rule** (added Improvement Loop 13, item D-1; redesigned in the item D-1 REWORK, 2026-09-14, after loose sentence-level co-occurrence produced ~50% false positives on the real corpus): for briefings dated `>= 2026-09-15`, when `pipeline.scoreChangesApplied` is `0` or absent, `headline`, `title`, `summary`, and each `topSignals[].title`/`.whyItMatters` may not narrate a score movement as already-happened fact — but only when a movement verb is directly BOUND to a score subject or score-shaped object (score-subject-verb, verb-plus-point-quantity, verb-plus-score-value, verb-plus-band, or verb-plus-its-score; e.g. "falls 5.9 points", "climbs into the Functional band"), not merely co-occurring anywhere in the sentence (so "Ebola Climbs to 1,155 Cases" and "the death toll rose to 3,899" correctly pass). A negation check (not/never/no/neither/nor before the verb) and a qualifier check ("would", "proposed", "pending", "not yet applied", "already", etc.) can still clear a sentence. `topSignals[].status === "applied"` is likewise forbidden under the same condition. Briefings dated before the cutoff are never failed by this rule — see `.claude/agents/overnight-digest.md` "PUBLIC DAILY JSON RULES" for the full binding-pattern definitions, verb/qualifier lists, and worked examples, and `scanUnappliedScoreMovement`/`BINDING_PATTERNS`/`evaluateMovementSentence` in `site/scripts/lib/lint-rules.mjs` for the machine-enforced version.
