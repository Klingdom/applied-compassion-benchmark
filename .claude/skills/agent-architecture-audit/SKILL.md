<!--
VENDORED 2026-09-22 from Klingdom/everything-claude-code (fork of affaan-m/ECC), MIT licensed.
Upstream path: skills/agent-architecture-audit/SKILL.md

Why this one is vendored rather than used from user scope: it needed edits.
  1. `tools:` declared Read, Write, Edit, Bash, Grep, Glob -> reduced to Read, Grep, Glob, Bash.
     A diagnostic that can rewrite the thing it is diagnosing is how a review becomes a change
     nobody approved. Every write in this repo goes through AUTONOMY.md Sec 1b.
  2. "**MANDATORY for:**" -> "Recommended for ... advisory only". No imported file outranks the
     coordinator's own selection rules.

Do not sync this file from upstream without re-applying both edits.
-->
---
...
**Recommended for (this project: advisory only — the coordinator's selection rules in `CLAUDE.md` decide what runs):**
